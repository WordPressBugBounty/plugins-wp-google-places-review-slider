(function( $ ) {
	...
	$(function(){
		...
		// Custom sources icon URLs lookup
		var customSourcesIconUrls = {};
		if (adminjs_script_vars.customSourcesIconUrls) {
			try {
				customSourcesIconUrls = JSON.parse(adminjs_script_vars.customSourcesIconUrls);
			} catch(e) {
				console.error('Error parsing custom sources icon URLs:', e);
			}
		}
		var tempdataset={};
		var reviewids=[];
								...
								$.each( tempobj, function( key, value ) {
								  ...
								  //build html to add to page
								   if(typelower!="manual" && typelower!="submitted"){
									// Check if this is a custom source (case-insensitive lookup)
									var iconSrc = '';
									var foundCustomIcon = false;
									if (customSourcesIconUrls) {
										// Try exact match first
										if (customSourcesIconUrls[key]) {
											iconSrc = customSourcesIconUrls[key];
											foundCustomIcon = true;
										} else {
											// Try case-insensitive match
											for (var customKey in customSourcesIconUrls) {
												if (customKey.toLowerCase() === typelower) {
													iconSrc = customSourcesIconUrls[customKey];
													foundCustomIcon = true;
													break;
												}
											}
									...
									if (!foundCustomIcon) {
										...
										iconSrc = adminjs_script_vars.pluginsUrl+'/public/partials/imgs/'+typelower+'_small_icon.png';
									}
									temphtml1 = temphtml1 + '<div class="lh36"><img src="'+iconSrc+'" alt="'+typelower+' logo" class="charttypelogo">'+key+' </div>';
								  } else {
		...
		var myVolumeChart;
		var myPlatformChart;
		var myPlatformVolumeChart;
		var myRatingTrendsChart;
		var WPRSP_DEBUG = true;

		function log(){
			if(!WPRSP_DEBUG){return;}
			try{ console.log.apply(console, arguments); }catch(e){}
		}
		function group(name){ if(WPRSP_DEBUG && console.groupCollapsed){ console.groupCollapsed(name);} }
		function groupEnd(){ if(WPRSP_DEBUG && console.groupEnd){ console.groupEnd(); } }

		// Initialize new charts when page loads
		loadVolumeChart();
		loadPlatformChart();
		loadPlatformVolumeChart();
		loadRatingTrendsChart();

		// Function to load review volume timeline chart
		function loadVolumeChart() {
			$("#volumeChartspinner").show();
			
			var stringseltypes = JSON.stringify(seltypesarray);
			var stringsellocation = JSON.stringify(sellocationsarray);
			var filterbytext = $("#wprevpro_analytics_filter_string").val();
			group('[WPRSP] Load Volume Chart');
			log('payload', {seltypes: stringseltypes, sellocations: stringsellocation, filtertext: filterbytext, startdate: startdate, enddate: enddate});
			
			var ajaxUrl = (typeof adminjs_script_vars !== 'undefined' && adminjs_script_vars.ajax_url) ? adminjs_script_vars.ajax_url : (typeof ajaxurl !== 'undefined' ? ajaxurl : '/wp-admin/admin-ajax.php');
			$.ajax({
				url: ajaxUrl,
				type: 'POST',
				data: {
					action: 'wprevpro_analytics_volume',
					seltypes: stringseltypes,
					sellocations: stringsellocation,
					filtertext: filterbytext,
					startdate: startdate,
					enddate: enddate
				},
				success: function(response) {
					...
					if (response && response.success) {
						buildVolumeChart(response.data);
					}
				},
				error: function() {
					console.log('Error loading volume chart data');
				}
			});
			groupEnd();
		}

		// Function to build volume chart
		function buildVolumeChart(data) {
			...
			if(!volumeEl){ log('[WPRSP] volumeChart canvas not found'); return; }
			var volumeCtx = volumeEl.getContext('2d');
			log('[WPRSP] Chart.js version', (typeof Chart!=='undefined'&&Chart.version)?Chart.version:'n/a');
			log('[WPRSP] buildVolumeChart labels/data', data && data.labels ? data.labels.length : 'n/a', data && data.data ? data.data.length : 'n/a');
			...
			$("#volumeChartspinner").hide();
			// click to open review list for selected day
			$('#volumeChart').off('click').on('click', function(e){
				var points = myVolumeChart.getElementsAtEvent(e);
				if(points && points.length>0){
					var idx = points[0]._index;
					var label = (data && data.date_keys && data.date_keys[idx]) ? data.date_keys[idx] : null;
					// open thickbox and render matching reviews by day
					var url = "#TB_inline?width=auto&height=auto&inlineId=tb_content_popup";
					tb_show(adminjs_script_vars.msg12, url);
					$("#TB_window").css({ "width":"80%","margin-left": "-40%","height":"80vh" });
					$("#TB_ajaxContent").css({ "width":"auto","height":"auto","max-height":"62vh","overflow":"auto" });
					$("#TB_window").focus();
					$("#review_details").hide();
					$("#review_list").show();
					var reviewshtml='';
					$.each( tempdataset.reviews, function( key, value ) {
					// Align review date to site timezone the same way as server did when bucketing
					// Use the textual created_time as stored (no timezone shift)
					var revKey = moment(value.created_time).format('YYYY-MM-DD');
						if(label && revKey===label){
							reviewshtml += getreviewshtml(value);
						}
					});
					$("#review_list_body").html(reviewshtml);
				}
		...
		// Function to load platform rating comparison chart
		function loadPlatformChart() {
			$("#platformChartspinner").show();
			
			var stringseltypes = JSON.stringify(seltypesarray);
			var stringsellocation = JSON.stringify(sellocationsarray);
			var filterbytext = $("#wprevpro_analytics_filter_string").val();
			group('[WPRSP] Load Platform Rating Chart');
			log('payload', {seltypes: stringseltypes, sellocations: stringsellocation, filtertext: filterbytext, startdate: startdate, enddate: enddate});
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					action: 'wprevpro_analytics_platform',
					seltypes: stringseltypes,
					sellocations: stringsellocation,
					filtertext: filterbytext,
					startdate: startdate,
				...
				success: function(response) {
					log('response', response);
					if (response && response.success) {
						buildPlatformChart(response.data);
					}
				},
				error: function() {
					console.log('Error loading platform chart data');
				}
			});
			groupEnd();
		}

		// Function to build platform chart
		function buildPlatformChart(data) {
			if (myPlatformChart) {
				myPlatformChart.destroy();
			}
			
			var platformEl = document.getElementById('platformChart');
			if(!platformEl){ log('[WPRSP] platformChart canvas not found'); return; }
			var platformCtx = platformEl.getContext('2d');
			log('[WPRSP] buildPlatformChart labels/data', data && data.labels ? data.labels.length : 'n/a', data && data.data ? data.data.length : 'n/a');
			myPlatformChart = new Chart(platformCtx, {
				type: 'bar',
				data: {
					labels: data.labels,
					datasets: [{
			...
			$("#platformChartspinner").hide();
			// click to open review list for selected platform
			$('#platformChart').off('click').on('click', function(e){
				var points = myPlatformChart.getElementsAtEvent(e);
				if(points && points.length>0){
					var idx = points[0]._index;
					var platformLabel = myPlatformChart.data.labels[idx];
					var url = "#TB_inline?width=auto&height=auto&inlineId=tb_content_popup";
					tb_show(adminjs_script_vars.msg12, url);
					$("#TB_window").css({ "width":"80%","margin-left": "-40%","height":"80vh" });
					$("#TB_ajaxContent").css({ "width":"auto","height":"auto","max-height":"62vh","overflow":"auto" });
					...
					$("#review_list").show();
					var reviewshtml='';
					var plabelLower = (platformLabel||'').toString().toLowerCase();
					$.each( tempdataset.reviews, function( key, value ) {
						var typeLower = (value.type||'').toString().toLowerCase();
						if(typeLower===plabelLower){
							reviewshtml += getreviewshtml(value);
						}
					});
					$("#review_list_body").html(reviewshtml);
		...
		// Function to load platform volume distribution chart
		function loadPlatformVolumeChart() {
			$("#platformVolumeChartspinner").show();
			
			var stringseltypes = JSON.stringify(seltypesarray);
			var stringsellocation = JSON.stringify(sellocationsarray);
			var filterbytext = $("#wprevpro_analytics_filter_string").val();
			group('[WPRSP] Load Platform Volume Chart');
			log('payload', {seltypes: stringseltypes, sellocations: stringsellocation, filtertext: filterbytext, startdate: startdate, enddate: enddate});
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					action: 'wprevpro_analytics_platform_volume',
					seltypes: stringseltypes,
					sellocations: stringsellocation,
					filtertext: filterbytext,
					startdate: startdate,
				...
				success: function(response) {
					log('response', response);
					if (response && response.success) {
						buildPlatformVolumeChart(response.data);
					}
				},
				error: function() {
					console.log('Error loading platform volume chart data');
				}
			});
			groupEnd();
		}

		// Function to build platform volume chart
		function buildPlatformVolumeChart(data) {
			if (myPlatformVolumeChart) {
				myPlatformVolumeChart.destroy();
			}
			
			var platformVolumeEl = document.getElementById('platformVolumeChart');
			if(!platformVolumeEl){ log('[WPRSP] platformVolumeChart canvas not found'); return; }
			var platformVolumeCtx = platformVolumeEl.getContext('2d');
			log('[WPRSP] buildPlatformVolumeChart labels/data', data && data.labels ? data.labels.length : 'n/a', data && data.data ? data.data.length : 'n/a');
			myPlatformVolumeChart = new Chart(platformVolumeCtx, {
				type: 'doughnut',
				data: {
					labels: data.labels,
					datasets: [{
			...
			$("#platformVolumeChartspinner").hide();
			// click to open review list for selected platform (doughnut)
			$('#platformVolumeChart').off('click').on('click', function(e){
				var points = myPlatformVolumeChart.getElementsAtEvent(e);
				if(points && points.length>0){
					var idx = points[0]._index;
					var platformLabel = myPlatformVolumeChart.data.labels[idx];
					var url = "#TB_inline?width=auto&height=auto&inlineId=tb_content_popup";
					tb_show(adminjs_script_vars.msg12, url);
					$("#TB_window").css({ "width":"80%","margin-left": "-40%","height":"80vh" });
					$("#TB_ajaxContent").css({ "width":"auto","height":"auto","max-height":"62vh","overflow":"auto" });
		...
		// Function to load rating trends chart
		function loadRatingTrendsChart() {
			$("#ratingTrendsChartspinner").show();
			
			var stringseltypes = JSON.stringify(seltypesarray);
			var stringsellocation = JSON.stringify(sellocationsarray);
			var filterbytext = $("#wprevpro_analytics_filter_string").val();
			group('[WPRSP] Load Rating Trends Chart');
			log('payload', {seltypes: stringseltypes, sellocations: stringsellocation, filtertext: filterbytext, startdate: startdate, enddate: enddate});
			...
			$.ajax({
				...
				type: 'POST',
				data: {
					action: 'wprevpro_analytics_rating_trends',
					seltypes: stringseltypes,
					sellocations: stringsellocation,
					filtertext: filterbytext,
					startdate: startdate,
				...
				success: function(response) {
					log('response', response);
					if (response && response.success) {
						buildRatingTrendsChart(response.data);
					}
				},
				error: function() {
					console.log('Error loading rating trends chart data');
				}
			});
			groupEnd();
		}

		// Function to build rating trends chart
		function buildRatingTrendsChart(data) {
			if (myRatingTrendsChart) {
				myRatingTrendsChart.destroy();
			}
			
			var ratingTrendsEl = document.getElementById('ratingTrendsChart');
			if(!ratingTrendsEl){ log('[WPRSP] ratingTrendsChart canvas not found'); return; }
			var ratingTrendsCtx = ratingTrendsEl.getContext('2d');
			log('[WPRSP] buildRatingTrendsChart labels/data', data && data.labels ? data.labels.length : 'n/a', data && data.data ? data.data.length : 'n/a');
			myRatingTrendsChart = new Chart(ratingTrendsCtx, {
				type: 'line',
				data: {
					labels: data.labels,
					datasets: [{
			...
			$("#ratingTrendsChartspinner").hide();
			// click to open review list for selected week
			$('#ratingTrendsChart').off('click').on('click', function(e){
				var points = myRatingTrendsChart.getElementsAtEvent(e);
				if(points && points.length>0){
					var idx = points[0]._index;
					// Use week_keys from server (YYYY-MM-DD Monday)
					var weekKey = (data && data.week_keys && data.week_keys[idx]) ? data.week_keys[idx] : undefined;
					var weekStart = moment(weekKey, 'YYYY-MM-DD');
					var url = "#TB_inline?width=auto&height=auto&inlineId=tb_content_popup";
					tb_show(adminjs_script_vars.msg12, url);
					$("#TB_window").css({ "width":"80%","margin-left": "-40%","height":"80vh" });
					$("#TB_ajaxContent").css({ "width":"auto","height":"auto","max-height":"62vh","overflow":"auto" });
					...
					var reviewshtml='';
					$.each( tempdataset.reviews, function( key, value ) {
						// Compute the Monday (ISO) of the review's week to match server key
						var revWeekKey = moment(value.created_time).startOf('isoWeek').format('YYYY-MM-DD');
						if(weekKey && revWeekKey===weekKey){
							reviewshtml += getreviewshtml(value);
						}
					});
					$("#review_list_body").html(reviewshtml);
		...
		$('#location_multiple_select, #type_multiple_select, #wprevpro_analytics_filter_string').on('change', function() {
			seltypesarray = $('#type_multiple_select').val();
			sellocationsarray = $('#location_multiple_select').val();
